...
<html>
    ...
    <body>
        ...
        <?php
        if ($_SESSION['police']) {
            echo '<div class = "add_pts">
              <input type="number" name = "points" id = "points" class="add_pts_input">
              <div class = "add_pts_button mdl-button mdl-js-button mdl-button--raised">Add points</div>
              <div class = "add_pts_response"></div>
            </div>';
        }
        ?>
        ...
        <script>
            ...
            function send() {
                ...
                if (document.querySelector('.chat').value != "") {
                    ...
                    const params = "user=" + '<?php echo $email; ?>' + "&chatContent=" + document.querySelector('.chat').value + "&time=" + time + "&fullDate=" + fullDate + "&to=" + '<?php echo $to; ?>';
            ...
            function getChats(){
                ...
                const params = "user=" + '<?php echo $email; ?>' + "&to=" + '<?php echo $to; ?>';
            ...
            <?php
              if ($_SESSION['police']) {
                echo "document.querySelector('.add_pts_button').addEventListener('click', addPts);";
              }
             ?>

             function addPts() {
               if (document.getElementById('points').value > 0) {
                 const params = "to=" + '<?php echo $to; ?>' + "&points=" + document.getElementById('points').value;

                 var xhttp = new XMLHttpRequest();
                 var url = "addPoints.php";
                 xhttp.open("POST", url, true);

                 xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                 xhttp.onreadystatechange = function() {
                     if (this.readyState == 4 && this.status == 200) {
                        document.querySelector(".add_pts_response").innerHTML = xhttp.responseText;
                     }
                 };
                 xhttp.send(params);
               }
